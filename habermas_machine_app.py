...
if __name__ == "__main__":
    ...
    def generate_text(self, prompt, temperature=0.2, max_tokens=2048, stream=False):
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error generating text: Status code {response.status_code}")
                return None
    ...
    def summarize_feedback(self, results, proposal_title, proposal_text, representative_statements=None, top_concerns=None):
        """Generate a summary of feedback results"""
...
The summary should follow Habermas Machine principles:
...
5. Make content accessible and clearly organized
...
"""
        ...
        if top_concerns:
            ...
            for concern, count in top_concerns:
                prompt += f"- {concern}: {count} mentions\n"
            prompt += "\n"
        ...
        prompt += """
Create a comprehensive summary using Markdown formatting with the following sections:
1. An overall assessment of the reception (positive, negative, or mixed)
2. Quick statistics on the sentiment breakdown
3. Key themes and concerns identified
4. Important insights from the data
5. Representative quotes that illustrate different perspectives

Be balanced and focus on finding consensus while accurately representing diverse perspectives.
"""
        ...
        # Clean up the summary
        if summary:
            summary = re.sub(r'<.*?>', '', summary)  # Remove any XML tags
            if not summary.startswith('#'):
                summary = f"# Feedback Summary for: {proposal_title}\n\n{summary}"
