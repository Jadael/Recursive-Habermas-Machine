...
class HabermasLLMSummarizer:
    """
    A class to generate LLM-powered summaries that respect Habermas Machine principles
    by heavily weighting participants' own words while making content more readable
    """
    ...
    def generate_summary(self, results, proposal_title, proposal_text, representative_statements=None, top_concerns=None, department_insights=None):
        """
        Generate an LLM-powered summary that respects the original wording of participants
        ...
        if not self.available or not results:
            return self._generate_fallback_summary(results, proposal_title, top_concerns, representative_statements)
    ...
    def _prepare_summary_prompt(self, results, proposal_title, proposal_text, representative_statements, top_concerns, department_insights, favorable_pct, neutral_pct, unfavorable_pct):
...
"""
    ...
    def _generate_fallback_summary(self, results, proposal_title, top_concerns=None, representative_statements=None):
    ...
    def generate_suggestions_summary(self, results, suggestion_counts, suggestion_statements, categories):
        """Generate an enhanced summary of suggestions with the LLM"""
        if not self.available or not results or not suggestion_counts:
            return self._generate_fallback_suggestions(suggestion_counts, suggestion_statements, categories)
            
        try:
            # Prepare prompt for LLM
            prompt = self._prepare_suggestions_prompt(results, suggestion_counts, suggestion_statements, categories)
    ...
    def _prepare_suggestions_prompt(self, results, suggestion_counts, suggestion_statements, categories):
...
"""
    ...
    def _generate_fallback_suggestions(self, suggestion_counts, suggestion_statements, categories):
